...
public class SearchCrawler extends JFrame {
	...
	private static final String[] MAX_URLS = { "50", "100", "500", "1000" }; // 콤보박스에
	// 표시할
	// 값들
	// ->
	// 최대
	...
	public SearchCrawler() {
		...
		// Set up file menu.
		// 최상단의 File을 누르면 EXIT가 나오는데 이것도 UI를 끝내는데 이용할 수 있게하는 설정
		...
		while (crawling && toCrawlList.size() > 0) { 
			...
			if (pageContents != null && pageContents.length() > 0) { //페이지의 내용이 있을 경우 
				...
				ArrayList<String> links = retrieveLinks(verifiedUrl, pageContents,
						crawledList, limitHost); //limitHost??? 아무데도 설정 되어 있지 않는데 잘 모르겠다
